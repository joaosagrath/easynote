...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $verifica_sql = "SELECT * FROM retirada WHERE id_professor = '$id_professor' AND sala = '$sala' AND rp = '$rp' AND status = 'Em Uso'";
    ...
    if ($verifica_result->num_rows > 0) {
        ...
        $atualiza_retirada = "UPDATE retirada SET controle_ar = '$controleAr', controle_projetor = '$controleProjetor', marcadores = '$marcadores', apagador = '$apagador', dataout = '$dataOut', status = '$statusFinalizado' WHERE id_professor = '$id_professor' AND sala = '$sala' AND rp = '$rp' 
		AND status = 'Em Uso'";
